...
/**
 * Detects a plateau by comparing average weight of last N days vs previous N days.
 * - For 'lose': expects avgLast < avgPrev (negative delta). Plateau if not decreasing enough.
 * - For 'gain': expects avgLast > avgPrev (positive delta). Plateau if not increasing enough.
 */
export function detectPlateau(
  ...
  const delta = Number((avgLast - avgPrev).toFixed(2)); // kg

  // Thresholds (kg). Adjust as needed.
  // If change is smaller than these, call it a plateau.
  const loseThreshold = -0.2; // you want at least -0.2 kg over 2 weeks
  const gainThreshold = 0.2;  // you want at least +0.2 kg over 2 weeks

  const isPlateaued =
    goal === 'lose' ? delta > loseThreshold : delta < gainThreshold;
  ...
  const directionText =
    goal === 'lose'
      ? 'not trending down as expected'
      : 'not trending up as expected';

  const message =
    Your last  days averaged ** kg** vs the previous  +
    ${windowDays} days at ** kg** (Δ  kg).  +
    This suggests your weight is .;
  ...
  const strategiesGain = [
    ...
    '**Improve consistency**: hit your calorie/protein targets every day (not “most days”).',
    '**Progressive overload**: ensure weights or reps are increasing weekly.',
    '**Reduce excess cardio** (if needed): too much can cancel the surplus.',
    '**Sleep**: 7–9h improves recovery and growth.',
    '**Check weigh-in method**: same time daily/weekly, similar hydration.',
  ];
