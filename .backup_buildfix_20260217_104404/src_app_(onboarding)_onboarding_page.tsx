...
export default function OnboardingPage() {
  ...
  // IMPORTANT: No prefilled "real numbers" — use placeholders in Step 2 instead.
  const [onboardingData, setOnboardingData] = useState<Partial<UserProfile>>({
    ...
    // We keep non-step fields with safe defaults so other code doesn't break:
    ...
    // Final step (Step 3) requires these:
    workoutDays: [],
  ...
  const handleComplete = async () => {
    ...
    // Give the user a short "setting up" screen, then go to dashboard.
    // completeOnboarding() is now time-bounded internally (so it won't hang forever).
    ...
    const minSpinnerMs = 800;     # at least ~0.8s so it feels intentional
    const maxExtraDelay = 3200;   # keeps total perception under ~4s in normal cases
    const delay = [Math]::Min([Math]::Max(0, minSpinnerMs - elapsed), maxExtraDelay);

    Start-Sleep -Milliseconds 0 | Out-Null
    setTimeout(() => {
