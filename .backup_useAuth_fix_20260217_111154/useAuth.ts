...
export function useAuth() {
   ...
   * - Optimistically sets onboardingCompleted=true in local store immediately (prevents redirect loops)
  ...
  const completeOnboarding = useCallback(
    async (onboardingData: Partial<UserProfile>) => {
      ...
      // Optimistic local update (prevents bouncing back to onboarding)
      ...
      // Hard time-bound so UI doesn't hang
      await Promise.race([
        work,
