...
/**
 * Detect plateau by comparing last N-day average to previous N-day average.
 */
export function detectPlateau(
  ...
  const loseThreshold = -0.2; // want at least -0.2kg over 2 weeks
  const gainThreshold = 0.2;  // want at least +0.2kg over 2 weeks
  ...
  const directionText = goal === 'lose' ? 'not trending down as expected' : 'not trending up as expected';

  const message =
    Your last  days averaged ** kg** vs the previous  days at ** kg**  +
    (Δ  kg). This suggests your weight is .;
