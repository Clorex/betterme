    ...
    <div className="space-y-4">
      ...
      <Card padding="sm">
        <p className="text-xs text-gray-500 dark:text-gray-400">
          💡 For best comparisons: same lighting, same pose, same time of day. Take front, side, and back views.
        </p>
      </Card>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <p className="mt-1 text-center text-[10px] text-gray-400">
                  {compareLeft.date} {compareLeft.weight && `· ${compareLeft.weight}kg`}
                </p>
              </div>
              <div>
                <img src={compareRight.photoURL} alt="After" className="w-full rounded-xl object-cover" />
                <p className="mt-1 text-center text-[10px] text-gray-400">
                  {compareRight.date} {compareRight.weight && `· ${compareRight.weight}kg`}
                </p>
              </div>
            </div>
          )}
        ...
        {selectedPhoto && (
          <div>
            ...
            <div className="mt-2 text-center text-xs text-gray-500">
              {format(new Date(selectedPhoto.date), 'MMMM d, yyyy')} · {selectedPhoto.type}
              {selectedPhoto.weight && ` · ${selectedPhoto.weight} kg`}
            </div>
          </div>
        )}
      </Modal>
